...
describe("processingPaths", () => {
  ...
  it("builds base path under systems/<slug>", () => {
    expect(norm(paths.base)).toBe(
      "/home/user/.machine-violet/systems/d-d-5e",
    );
  });

  it("builds pipeline state path", () => {
    expect(norm(paths.pipelineState)).toContain("systems/d-d-5e/pipeline.json");
  });

  it("builds catalog path", () => {
    expect(norm(paths.catalog)).toContain("systems/d-d-5e/catalog.json");
  });
  ...
  it("builds index and cheat sheet paths", () => {
    expect(norm(paths.index)).toContain("systems/d-d-5e/index.md");

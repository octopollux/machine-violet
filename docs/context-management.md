...
The **setup agent** gets the same sample for the same reason — its "surprise me" character generation otherwise collapses to a handful of names every campaign (a live sweep produced "Mara Vale" verbatim ~7 times across 67 seeds). Unlike the DM's permissive framing, setup is explicitly told: "When inventing names, use names from the Name Inspiration lists." `buildSystemPrompt()` in `setup-conversation.ts` calls `buildSetupNameInspiration()`, which shares the same sampling implementation while rendering that setup-only instruction, and pushes the resulting `## Name Inspiration` block into **Tier 3** (the per-session section alongside the shuffled seeds/personalities), not the 1h-cache-stamped Tier 1/2 prefix. A fresh sample is drawn per session regardless of tier (`buildSystemPrompt` runs once per session); Tier 3 placement keeps this per-session-random content out of the cross-session-stable cached prefix so it doesn't churn those cache hits — the same reason the shuffled seeds live there.
...
**Prompt framing**: the rendered block opens with "AI agents like you tend to favor the same names when creating characters". The DM sees "For inspiration only — don't feel bound to this list", which keeps it from treating the list as authoritative. The setup agent instead receives the explicit instruction above, because its one-time campaign setup role benefits from choosing names from the supplied sample.

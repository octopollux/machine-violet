...
**Per-campaign state is owned by the playing phase.** Everything that describes *which* campaign you're in — the narrative log, the theme/variant/key colour, `clientState`, and the campaign identity (`campaign` in `app.tsx`: id + human-readable name, see `campaign-identity.ts`) — is set on every entry into the playing phase (`startCampaign`, New Campaign, the setup→game handoff) and cleared on the way back out (`returnToMenu`). Both halves are load-bearing, because setup renders inside `PlayingPhase` *without ever receiving a `state:snapshot`*: whatever the last session left behind is what the new campaign's setup screen labels itself with. This was a real bug — loading a campaign, exiting, and choosing New Campaign left the previous campaign's name (or its slug) in the top frame and modeline for the whole setup conversation. The regression gate is `packages/test-harness/bin/campaign-identity-probe.ts` (offline, tape replay).

A related trap on the same seam: the server broadcasts one last `state:snapshot` as part of `endSession()` teardown, and that one carries `campaignName: ""` with `players: []`. It can land *after* the client's own reset, so client code that reads those fields treats empty as absent rather than as an authoritative override (`campaignLabel`, and the player-list fallback in `PlayingPhase`).
...
**Connection detail** (`ConnectionDetail`) — provider, health status (verbatim message), masked key, endpoint, usage, and actions: **Use this connection** (disabled with "already in use" when active), **Check connection**, **Delete connection**. When the last check failed (invalid/error), a **Fix connection** action leads the list: key-based providers open `FixConnection` — re-enter the key masked, `PATCH /manage/connections/:id` replaces it **in place** (same id, tier assignments kept), then the same validate-on-submit contract as the wizard (invalid stays with the provider's error; verified pops back to detail) — while ChatGPT connections re-run the OAuth sign-in (`ChatGptSignIn`; the server upserts the existing connection). Env-var connections get a disabled Fix with "update the environment variable, then check again". Delete needs a second Enter to confirm; env-var connections show *why* they can't be deleted ("set by an environment variable — remove it from your environment instead") instead of silently refusing. Action failures surface inline in red.

**Health checks probe a real model.** The check route passes the provider's small-tier registry default (else the connection's first discovered model) to `healthCheck` — without it, providers sharing the OpenAI-compatible client (xAI, OpenRouter, custom) were probed with an api.openai.com model id and reported 400 "Model not found" on perfectly valid keys.
...
**Sign in with ChatGPT** (wizard step) — OAuth via the codex app-server (see [openai-chatgpt-provider.md](openai-chatgpt-provider.md)). Renders as a `CenteredModal` overlay (title "Sign in with ChatGPT", hints in the modal footer, body padded to ~60% of terminal height for opacity) over a blank Connect to AI frame; the long auth URL wraps to the modal's inner width. States: starting → pending (auth URL, `o open in browser · c copy URL · Esc cancel`, 2 s status poll) → success (`✔ Signed in as <email> (<plan>)`, straight into the wizard result) / cancelled / error. Esc while pending cancels server-side. Hint rule: every not-yet-successful state (starting, pending, cancelled, failed, transport error) hints **Esc** as the way out; only a verified successful sign-in switches to an Enter-only hint (the result screen's `Enter continue`).
...
**Code:** `packages/client-ink/src/phases/connections/` — `ConnectionsArea.tsx` (stack + data), `ConnectionsList.tsx`, `ConnectionDetail.tsx`, `ModelAssignments.tsx`, `ConnectWizard.tsx`, `ChatGptSignIn.tsx` (OAuth screen shared by wizard + Fix), `FixConnection.tsx` (in-place key re-entry), `providers.ts` (all provider-facing copy), `usage-format.ts`.

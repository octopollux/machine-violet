...
**Sign in with ChatGPT** (wizard step) — OAuth via the codex app-server (see [openai-chatgpt-provider.md](openai-chatgpt-provider.md)). States: starting → pending (auth URL, `o open in browser · c copy URL · Esc cancel`, 2 s status poll) → success (`✔ Signed in as <email> (<plan>)`, straight into the wizard result) / cancelled / error. Esc while pending cancels server-side. Hint rule: every not-yet-successful state (starting, pending, cancelled, failed, transport error) hints **Esc** as the way out; only a verified successful sign-in switches to an Enter-only hint (the result screen's `Enter continue`).

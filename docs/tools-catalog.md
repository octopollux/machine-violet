...
| `set_display_resources` | T1 | DM, Setup | `({ character, resources[] })` | Update which resource keys appear in the top frame for a character. Also stores keys on `GameState.displayResources`. Its executable `durable` contract accepts a bare or comma-separated string only through the tool-local `csv_to_string_array` repair, then validates the canonical `string[]` before dispatch. Legacy persisted state remains defensively normalized by shared consumers. |

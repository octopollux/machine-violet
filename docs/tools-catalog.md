...
| `set_display_resources` | T1 | DM, Setup | `({ character, resources[] })` | Update which resource keys appear in the top frame for a character. Also stores keys on `GameState.displayResources`. `resources` is coerced through `coerceResourceKeys` (`shared/utils/resource-keys.ts`): a bare string is accepted and split on commas, since neither provider enforces the `array` schema and the sheet's `display_resources` front matter is comma-separated. |

...
The encrypted-blob round-trip still works on the `openai-apikey` / `openrouter` paths directly against the Responses API. See issue #533 for the wider cross-provider context (Anthropic uses its own `thinking` + `redacted_thinking` blocks with `signature`; `custom` Chat-Completions endpoints have no equivalent contract) and #607 for the codex-path no-op finding and the open upstream question.

...
| `codex:subprocess:reset` | `{ code, signal, sessionId? }` | The active client exited on its own (not via `dispose()`); we dropped the reference so the next call respawns a fresh subprocess. Without this the dead client stayed latched and every later call threw "codex app-server has exited" — the unrecoverable-first-turn-crash bug. |
| `codex:turn:retry_after_exit` | `{ code, signal, sessionId? }` | A turn's subprocess died **before any MV tool ran**, so no side effects persisted; the provider respawns and replays the turn once, transparently. Bounded to a single retry; a post-tool death is NOT auto-retried. |
...
| `codex:rpc:reasoning_item_missing` | `{ threadId, turnId, summaryDeltas, sessionId? }` | **Once per session.** Model reasoned but no `rawResponseItem/completed` reasoning item arrived. On a ChatGPT account this is the expected steady state — codex emits no raw reasoning items there at all (live-verified), so there is nothing to preserve cross-turn and the #533 codex replay was removed (#607). Kept as a tripwire: if such items ever start arriving, this stops firing. A genuine intermittent transport drop surfaces instead as a `parse_failure` with `methodGuess`. (#597) |
...
(There is no encrypted-reasoning *capture* on this provider: codex surfaces no usable blob on the ChatGPT-account path, so the #533 codex replay was removed as a no-op (#607). The one diagnostic that remains is `codex:rpc:reasoning_item_missing` — see Reasoning preservation below, and issues #597/#607.)

## Reasoning preservation across turns

**Not preserved on this provider.** On the Responses API a reasoning model produces an opaque `encrypted_content` blob per reasoning item that the API accepts back as input on later turns — that's what lets the `openai-apikey` / `openrouter` / `xai` paths continue a chain-of-thought instead of re-deriving it each round. codex exposes that blob (when present) on a separate **`rawResponseItem/completed`** notification, not on the sanitized `item/completed` stream (whose `ReasoningThreadItem` view defines only `summary` and `content` text).

#533 / PR #538 wired the codex provider to capture that blob and replay it, mirroring the openai-apikey path. **A live test on a ChatGPT Plus account proved it is a no-op there: codex emits no `rawResponseItem/completed` reasoning items at all** — the `codex:rpc:reasoning_item_missing` tripwire fired on every reasoning turn (`summaryDeltas` > 0, `rawReasoningItems` = 0). codex only requests/forwards `reasoning.encrypted_content` for **ZDR** orgs (which can't use server-side response storage); a consumer ChatGPT account gets none, and there is no `thread/start` knob MV sends that flips it on. So the dead capture/replay machinery was **removed** (#607). The DM re-derives its reasoning each turn — graceful and invisible, since campaign secrets ride in `dm-notes` every turn regardless; only cross-turn reasoning *efficiency* is affected, not correctness.

What remains is the **tripwire**: the provider still subscribes to `rawResponseItem/completed` solely to count reasoning items and log `codex:rpc:reasoning_item_missing` once per session. If a ZDR/enterprise ChatGPT login ever surfaces real encrypted reasoning items, that breadcrumb stops firing — the signal to re-add capture+replay. The reasoning **summary** text is still captured and surfaced via `thinkingText` for the debug dump; it is not replayed. A genuine *intermittent* transport drop is a different signature — it surfaces as a `parse_failure` with a `methodGuess`, not as this once-per-session breadcrumb.

The encrypted-blob round-trip still works on the `openai-apikey` / `openrouter` / `xai` paths directly against the Responses API. See issue #533 for the wider cross-provider context (Anthropic uses its own `thinking` + `redacted_thinking` blocks with `signature`; `custom` Chat-Completions endpoints have no equivalent contract) and #607 for the codex-path no-op finding and the open upstream question.
...
## Turn failure modes & recovery

A turn must never hang the game. `runTurn` awaits a `completion` promise that resolves on `turn/completed` for its thread; three things can stop that ever happening, each converted into a thrown, classified error instead of an unbounded wait:

- **codex reports the turn failed.** `turn/completed` arrives with `status: "failed"`; we throw `CodexTurnFailedError`, whose `kind` is derived from the message by `classifyCodexFailure`. Quota exhaustion — `"rate limit"`, `429`, `"usage limit"`, `"quota"`, `"out of/insufficient credits"` — classifies as **`rate_limited`** and `classifyServerError` routes it to **`retryable`** (recoverable overlay, session preserved) rather than the drop-to-menu bucket the auth/model/tools kinds use. Quota is a normal, transient condition on this provider, so a mid-game turn that hits it is simply re-sendable once the window resets.
- **The subprocess dies mid-turn.** rpc.ts rejects in-flight `call()`s on `exit`, but `turn/start` has already returned by the time we await `completion`, so that promise isn't covered. `runTurn` subscribes to the client's `exit` and rejects `completion` with **`CodexProcessExitedError`** (carrying the exit `code`/`signal`) the instant codex goes away — otherwise a codex death after `turn/start` would hang forever. **Recovery is two-layered.** First, `ensureStarted` installs a provider-lifetime `exit` listener that drops the dead client (`this.rpc = null`, emits `codex:subprocess:reset`) — *without this the provider latched onto the corpse and every subsequent call, including the player's "Press Enter to retry", threw "codex app-server has exited" until relaunch, which is what made a first-turn `code=1` crash unrecoverable in place.* Second, `runTurn` **transparently auto-retries once**: it respawns via a fresh `ensureStarted` and replays the turn — but ONLY when no MV tool had dispatched this turn (`shouldAutoRetryTurn`), since a tool may have persisted side effects (scribe writes, a rendered image) that a blind replay could double-apply. A post-tool death, or a second consecutive death, escapes as `CodexProcessExitedError` → `classifyServerError` → **`retryable`**, so the manual retry overlay (which now respawns) can still recover. The heavy first turn — biggest prompt + history injection + first reasoning pass, before any tool runs — is both the likeliest place for codex to self-exit and exactly the case the auto-retry absorbs.
- **codex stays alive but goes silent.** On a *persistent* 429 codex backs off and retries **internally** (it's built to run unattended), emitting no failure and no `turn/completed` — potentially until the rate window resets (~an hour). A per-turn **stall watchdog** (`StallWatchdog`, `TURN_STALL_TIMEOUT_MS` = 120 s) bounds this: any codex notification for the turn re-arms it, and it is **paused for the whole of every MV tool dispatch** (an image render runs minutes, a subagent tens of seconds — codex is correctly idle then) via a depth counter that survives concurrent tool calls. If codex itself goes dark past the window, it throws `CodexTurnStalledError` → `retryable`. So the only thing it ever measures is codex stalling while it owes us a turn; a healthy turn (which streams reasoning-summary deltas within seconds) never trips it.

## CODEX_HOME isolation (the `disk I/O error` crash)

codex initializes a single SQLite **state runtime** (`state_*.sqlite` + WAL, plus `goals_*`, `logs_*`, `memories_*`, `sessions/`) under its home dir at startup. When **multiple codex subprocesses share one home** (`~/.codex`) they contend on that DB, and on Windows the loser exits **`code=1`** with **`(code: 1546) disk I/O error`** (`SQLITE_IOERR_TRUNCATE`). Root-caused from `.debug/engine.jsonl`: a **4-wide simultaneous spawn** against one `~/.codex` killed **all four** (spawns within 17 ms of each other), while every staggered/sequential spawn in the same run survived — a pure startup race, not a first-turn-content problem (state init is the first thing codex does after spawn, so a contended spawn dies before turn 1). This is the dominant non-OOM crash.

**Fix — one home per subprocess.** Each `openai-chatgpt` provider is constructed with a unique `CODEX_HOME` under `<tmp>/machine-violet/codex-homes/` (`allocateCodexHome`, wired in `createProviderFromConnection`); the provider `mkdir`s it before spawn and `rm`s it on `dispose()`. Concurrent subprocesses then never share a state DB, so the contention is impossible. **Auth survives an empty home** because MV pushes ChatGPT tokens over RPC (`pushChatGptAuthTokens`), not from `<home>/auth.json` — verified live (`bin/codex-home-isolation.ts`: a fresh empty home authenticates and 4/4 concurrent isolated homes pass). Isolation is applied **only when a token store is present** (the ChatGPT-account flow); the no-token-store fallback keeps the default `~/.codex` because it authenticates from that home's `auth.json`.

**Cleanup.** `dispose()` removes a session's home in the normal case, so nothing accumulates. A one-time, best-effort **date-sweep** (`sweepStaleCodexHomes`, fired once per process from the factory) is the backstop for homes leaked by a crash/kill that skipped `dispose()` — it reaps homes whose **freshest inner-file mtime** (the WAL, rewritten every turn) is older than `CODEX_HOME_STALE_MS` (3 days), keying off the child mtime so an active long-idle session is never reaped out from under itself.

**Handoff ordering.** Setup and the game session run **separate** codex subprocesses (each tier resolution builds its own provider cache). With per-home isolation they no longer share a SQLite state runtime, so that half of the handoff race is gone — but they still share a **single-use OAuth refresh token** (mediated by MV's connection-backed token store). `SessionManager.transitionToGame` therefore **awaits** the setup session's `dispose()` (which awaits the subprocess `exit`) **before** starting the game session, so the two never overlap and can't both try to rotate the refresh token (a 401 "refresh token has already been used" at handoff).

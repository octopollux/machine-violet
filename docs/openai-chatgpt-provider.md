...
- **The subprocess dies mid-turn.** rpc.ts rejects in-flight `call()`s on `exit`, but `turn/start` has already returned by the time we await `completion`, so that promise isn't covered. `runTurn` subscribes to the client's `exit` and rejects `completion` itself the instant codex goes away — otherwise a codex death after `turn/start` would hang forever.

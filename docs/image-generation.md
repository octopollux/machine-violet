...
`dispatchUpdatePortrait` (`game-engine.ts`) re-renders the portrait via `provider.generateImage`, image-to-image **conditioned on the character's current portrait** (so identity carries forward and only the change applies). Three deliberate properties:

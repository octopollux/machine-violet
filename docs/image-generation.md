...
**Cost / caveats.** Image-to-image is materially slower than text-to-image (gpt-image-2 backend cost — minutes, not ~1), and with a reference attached the model *occasionally* spends a reasoning pass analyzing the image and replies with text instead of calling `image_gen` at all; the bounded retry (see [Provider backends](#provider-backends)) rescues that, at the cost of a wasted attempt. Lowering the render turn's reasoning effort would curb the over-analysis, but `"low"` is the **floor** — the backend rejects `image_gen` at `reasoning.effort: "minimal"` (HTTP 400), so that lever isn't available. Net: reserve references for deliberate "show *this* character" moments; they're not free.

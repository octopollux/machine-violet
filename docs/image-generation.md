...
**Composition.** The image decodes + resizes once (sharp), then a painter registered with the sync-write combiner re-blits the visible band inside the same DEC-2026 synchronized-output frame Ink just drew — flicker-free. The band to paint is computed in one pass: `visibleBand` clips the footprint to the narrative viewport (so it never overflows the modeline above or the input line below), then `subtractOcclusion` trims the rows any open modal covers — the **same vertical-band machinery** handles both screen edges and modals, so a modal covering part of an image crops it to the free rows instead of hiding it whole (`OcclusionProvider` / `useRegisterOcclusion`, registered by `CenteredModal` + `OverlayPane`; the inline `ChoiceOverlay` registers nothing and leaves the image fully visible). A modal opening/closing bumps the occlusion store (`useOcclusionVersion`), which re-renders the image and re-encodes the trimmed band; the painter also hides live the same frame if its cached band is stale against current occluders, so there's never a frame of image-over-modal. Critically, the painter's vacated-row erase is passed the occluder spans and **skips any row a modal owns** — otherwise the erase (fired the instant a modal covers the image) would blank the modal where the image had been, which read as "modals hidden by image."

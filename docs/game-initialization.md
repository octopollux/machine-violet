...
All paths include a mandatory pre-finalize review where the agent reads back the full configuration and gets explicit confirmation. The engine also validates the core `finalize_setup` handoff fields before scaffolding (campaign identity, DM personality, player name, character name, character concept, and the DM handoff note). If a provider emits a partial tool object, setup returns a tool error naming the missing fields and lets the agent repair the call; it never turns an incomplete handoff into the fallback `Player` / `Adventurer` / `The Unknown` campaign identity or starts the DM without the player's setup context.

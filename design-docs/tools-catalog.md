...
| `set_ui_style` | T1 | DM, Engine | `({ style?, variant? })` | Switch variant (combat/exploration/ooc/levelup/dev). Still works for variant-only changes; `style_scene` preferred for full theme changes. |

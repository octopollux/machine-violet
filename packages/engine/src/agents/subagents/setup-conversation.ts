...
export function createSetupConversation(
  ...
  function handleFinalize(input: Record<string, unknown>): void {
    ...
    const isNullish = rawSystem === null
      || rawSystem.toLowerCase() === "null"
      || rawSystem.toLowerCase() === "none";
    const resolvedSystem = isNullish ? null : resolveSystemSlug(rawSystem as string);

...
describe("runProviderLoop onImageGenerated", () => {
  it("invokes onImageGenerated once per image_generated ContentPart, with intent", async () => {
  ...
  it("is not invoked when the turn produced no images (zero-cost in the common case)", async () => {
    const provider: LLMProvider = {
      providerId: "test",
      chat: vi.fn(async () => textResult("Just words today.")),
      stream: vi.fn(),
      healthCheck: vi.fn(),
    };
  ...
  it("awaits the handler before dispatching tool calls in the same turn", async () => {
    // Order matters: an image must be persisted before any tool result it
    // might be referenced by races back to the model. Using a deferred
    // promise inside the image handler proves the loop waits.
    let imageResolved = false;
    let toolDispatchedBeforeImageResolved = false;
    const assistantContent: ContentPart[] = [
      {
        type: "image_generated",
        id: "ig_2",
      ...
      { type: "tool_use", id: "t_1", name: "noop", input: {} },
    ];
    const provider: LLMProvider = {
      providerId: "test",
      chat: vi.fn(async () => ({
        text: "",
        toolCalls: [{ id: "t_1", name: "noop", input: {} }],
        usage: mockUsage(),
        stopReason: "end" as const,
        assistantContent,
      })),
    ...
    await runProviderLoop(provider, "system", [
      ...
      stream: false,
      maxToolRounds: 1,
      tools: [{ name: "noop", description: "", inputSchema: { type: "object" } }],
      toolHandler: () => {
        if (!imageResolved) toolDispatchedBeforeImageResolved = true;
        return { content: "ok" };
      },
    ...
    expect(toolDispatchedBeforeImageResolved).toBe(false);
  });
});

    ...
    openapi: {
      info: {
        ...
        description: [
          ...
          "4. **Play** — submit player actions and choice responses with `POST /session/turn/contribute` (set `fromChoice: true` and include `choiceResponse` provenance for choice UI responses). The DM narrates via WebSocket `narrative:chunk` / `narrative:complete` events.",
          ...
          "with `fromChoice: true` and, during gameplay, `choiceResponse`). The setup agent uses the flag to distinguish a real selection",
